...
func routes() *httprouter.Router {
	...
	//r.GET("/web/static/*filepath", hr.Handler(alice.
	//	New().
	//	ThenFunc(controller.Static)))

	// Serve cert files, no directory browsing
	r.GET("/.well-known/*filepath", hr.Handler(alice.
		New().
		ThenFunc(controller.Wellknown)))
	...
	//r.GET("/recentposts/:page", hr.Handler(alice.
	//	New().
	//	ThenFunc(controller.PostListGET)))
	//
	//r.GET("/posts", hr.Handler(alice.
	//	New().
	//	ThenFunc(controller.PostListGET)))

	r.GET("/post/:seourlparam/:id", hr.Handler(alice.
		New().
		ThenFunc(controller.PostDetailGet)))

	r.GET("/posts/:sort/:hashtag/:page", hr.Handler(alice.
		New().
		ThenFunc(controller.PostListGET)))

	r.GET("/posts/:sort/:hashtag", hr.Handler(alice.
		New().
		ThenFunc(controller.PostListGET)))

	r.GET("/search/:sort/:q/:page", hr.Handler(alice.
		New().
		ThenFunc(controller.SearchGET)))

	r.GET("/search/:sort/:q", hr.Handler(alice.
		New().
		ThenFunc(controller.SearchGET)))

	r.GET("/recentpopularposts/:fromdate/:mediatype/:page", hr.Handler(alice.
		New().
		ThenFunc(controller.RecentPopularPosts)))

	r.GET("/recentpopularhashtags/:fromdate/:page", hr.Handler(alice.
		New().
		ThenFunc(controller.RecentPopularHashtags)))

	r.GET("/postexturl/:id/:index", hr.Handler(alice.
		New().
		ThenFunc(controller.LoadExternalUrl)))




	//r.GET("/post/create", hr.Handler(alice.
	//	New(acl.DisallowAnon).
	//	ThenFunc(controller.NotepadCreateGET)))
	//r.POST("/post/create", hr.Handler(alice.
	//	New(acl.DisallowAnon).
	//	ThenFunc(controller.NotepadCreatePOST)))
	//r.GET("/post/update/:id", hr.Handler(alice.
	//	New(acl.DisallowAnon).
	//	ThenFunc(controller.NotepadUpdateGET)))
	//r.POST("/post/update/:id", hr.Handler(alice.
	//	New(acl.DisallowAnon).
	//	ThenFunc(controller.NotepadUpdatePOST)))
	//r.GET("/post/delete/:id", hr.Handler(alice.
	//	New(acl.DisallowAnon).
