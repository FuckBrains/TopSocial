...
var (
	...
	rootTemplate       string
	singleTemplates     map[string][]string
...
type Template struct {
	...
	SingleTemplates     	map[string][]string	`json:"Singles"`
...
func LoadTemplates(rootTemp string, childTemps []string,singleTemp map[string][]string) {
	rootTemplate = rootTemp
	childTemplates = childTemps
	singleTemplates = singleTemp
}
...
func (v *View) PrependBaseURI(s string) string {
	return stringhelpers.FastConcat([]string {v.BaseURI, s})
}
...
func (v *View) RenderSingle(w http.ResponseWriter, returnString bool) (result string){
	...
	//templateList := []string{v.Name, "partial/post"}

	// List of template names
	templateList := make([]string, 0)
	templateList = append(templateList, v.Name)
	templateList = append(templateList, singleTemplates[v.Name]...)
	...
	tname:= strings.Replace(filepath.Base(templateList[0]),".tmpl","",1)
	// Determine if there is an error in the template syntax
	templates, err := template.New(tname).Funcs(pc).ParseFiles(templateList...)
	...
	if(returnString){
		...
		err = tc.Funcs(pc).ExecuteTemplate(&tpl,  tname, v.Vars)
		if err != nil {
			log.Fatal(err.Error())
		...
		err = tc.Funcs(pc).ExecuteTemplate(w,  tname, v.Vars)
		if err != nil {
			http.Error(w, "Template File Error: " + err.Error(), http.StatusInternalServerError)
		}
		return ""
